...
class MyNotes {
  constructor() {
    if (document.querySelector("#my-notes")) {
      (axios__WEBPACK_IMPORTED_MODULE_0___default().defaults.headers.common["X-WP-Nonce"]) = universityData.nonce; // this is for authorization
  ...
  clickHandler(e) {
    if (e.target.classList.contains("delete-note") || e.target.classList.contains("fa-trash-o")) this.deleteNote(e); // e.target always points to the tag or element where the event took place.
  ...
  editNote(e) {
    ...
    if (thisNote.getAttribute("data-state") == "edi table") {
  ...
  makeNoteEditable(thisNote) {
    ...
    thisNote.setAttribute("data-state", "editable"); // 1st arg refers to or creates the attribute name "data-state", 2nd arg is the value
  }
  ...
  makeNoteReadOnly(thisNote) {
    ...
    /*1st arg is the name, 2nd is the value*/
  ...
  async deleteNote(e) {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().delete(universityData.root_url
      /*this is a very useful API method written in functions.php => wp_localize_script()*/
      + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"));
...
class Search {
  ...
  typingLogic() {
    if (this.searchField.value != this.previousValue) {
      // this condition allows the Spinnerwheel to keep running only when there has been changes to the search field.
      clearTimeout(this.typingTimer);

      if (this.searchField.value) {
        if (!this.isSpinnerVisible) {
          // this condition controls the property of whether the Spinner is visible or not, so the spinner wheel doesn't restart everytime there is a keystroke 
  ...
  async getResults() {
    ...
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(universityData.root_url
      /*this is a very useful API method written in functions.php => wp_localize_script()*/
